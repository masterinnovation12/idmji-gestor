/**
 * Script para añadir variables de entorno faltantes en Vercel
 * Solo añade las que faltan y corrige los targets
 */

const https = require('https');

const VERCEL_TOKEN = '2XsNXkVqUsnI1MbrOF2GSfiM';
const PROJECT_ID = 'prj_cOghTJRDQQijZApUoSxJBJQSw6C1';
const TEAM_ID = 'team_hn64tc8bCRGPKr0ZEDkeo58Z';

// Variables sensibles solo para production y preview (no development)
const sensitiveVars = [
    {
        ...
        type: 'sensitive',
        target: ['production', 'preview'] // Sin development
    },
    {
...
// Variables normales para todos los targets
const normalVars = [
    {
...
async function main() {
    console.log('🚀 Añadiendo variables de entorno faltantes a Vercel...\n');
    ...
    // Añadir variables sensibles (solo production y preview)
    console.log('📝 Añadiendo variables SENSIBLES (production, preview)...\n');
    ...
    // Añadir variables normales para development si no existen
    console.log('📝 Verificando variables para DEVELOPMENT...\n');
    for (const envVar of normalVars) {
        // Intentar añadir solo para development
        const devVar = {
            ...envVar,
            target: ['development']
        };
        
        console.log(`📝 Añadiendo: ${envVar.key} para development...`);
        
        try {
            const result = await addEnvVar(devVar);
            
            if (result.status === 200 || result.status === 201) {
                console.log(`   ✅ ${envVar.key} añadida para development`);
                successCount++;
            } else if (result.status === 409 || result.data?.error?.code === 'env_already_exists') {
                console.log(`   ⚠️  ${envVar.key} ya existe para development (se omitirá)`);
                skippedCount++;
            } else {
                console.log(`   ❌ Error: ${result.data?.error?.message || result.status}`);
                errorCount++;
