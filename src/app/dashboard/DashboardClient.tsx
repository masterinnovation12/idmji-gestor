...
function AssignmentPill({ label, usuario, lectura, himnario }: { label: string, usuario: Partial<Profile> | null | undefined, lectura?: any, himnario?: any[] }) {
    ...
    const tiempoLabel = () => {
        if (hasHimnos && hasCoros) return "Tiempo Himnos y Coros"
        if (hasHimnos) return "Tiempo Himnos"
        if (hasCoros) return "Tiempo Coros"
        return ""
    }
                ...
                <div className="flex items-center gap-3 p-3.5 bg-white/40 dark:bg-white/5 rounded-2xl border border-white/60 dark:border-white/10 shadow-sm relative group w-full min-w-0">
                    ...
                    <div className="min-w-0 flex-1 flex flex-col justify-center">
                        <div className="flex items-center flex-wrap gap-1.5 mb-1">
                            <span className="text-[9px] font-black uppercase tracking-widest text-blue-600 dark:text-blue-400">Lectura</span>
                            ...
                            <div className="flex items-center gap-1 bg-emerald-500/10 dark:bg-emerald-500/20 px-1.5 py-0.5 rounded-md border border-emerald-500/20">
                                ...
                                <span className="text-[8px] font-bold uppercase tracking-wider text-emerald-600 dark:text-emerald-400 whitespace-nowrap">Registrada</span>
                            </div>
                        </div>
                ...
                <div className={`flex flex-col gap-4 p-4 bg-white/40 dark:bg-white/5 rounded-2xl border border-white/60 dark:border-white/10 shadow-sm relative w-full ${lectura ? 'mt-1' : ''}`}>
                    ...
                    <div className="flex items-center gap-3 pb-3 border-b border-black/5 dark:border-white/5">
                        ...
                        <div className="flex-1">
                            <h3 className="text-[10px] font-black uppercase tracking-widest text-indigo-600 dark:text-indigo-400 leading-none">Himnario Alabanza</h3>
                            <p className="text-[9px] text-slate-400 font-bold mt-1 uppercase tracking-tight">{himnario.length} piezas seleccionadas</p>
                        </div>
                    </div>
                        ...
                        {himnario.map((item, idx) => {
                            ...
                            return (
                                <div key={idx} className="flex items-center justify-between gap-3 group/item p-2 hover:bg-white/50 dark:hover:bg-white/5 rounded-xl transition-all">
                                    ...
                                    <div className="shrink-0 font-mono text-[10px] font-bold text-slate-400 bg-slate-100/50 dark:bg-slate-800/50 px-2 py-1 rounded-md border border-black/5 dark:border-white/5">
                    ...
                    <div className="flex items-center justify-center mt-2 pt-3 border-t border-black/5 dark:border-white/5">
                        <div className="px-4 py-2 bg-slate-900 dark:bg-white rounded-2xl shadow-xl flex items-center gap-3">
                            ...
                            <span className="text-[10px] font-black text-slate-300 dark:text-slate-500 uppercase tracking-widest">
                                {tiempoLabel()}: <span className="text-white dark:text-slate-900 font-mono text-xs ml-1">{formatDuration(totalSeconds)}</span>
                            </span>
                        </div>
                    </div>
                </div>
