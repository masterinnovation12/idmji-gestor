...
export function AssignmentPill({ label, usuario, lectura, lecturas, himnario, tipoCulto, action, footerAction, temaIntroduccionAlabanza, nota, onEditReading }: AssignmentPillProps) {
    ...
    return (
        <div className={`flex p-3 sm:p-4 bg-white/70 rounded-3xl border border-[rgba(184,150,74,0.18)] transition-all shadow-sm relative ${hasExtraContent || hasFooter ? 'flex-col gap-3' : 'flex-row items-center gap-2 sm:gap-3'} ${hasExtraContent ? 'bg-[#f8f3e8]/40 border-[rgba(184,150,74,0.28)]' : ''}`}>
            {usuario ? (
                <div className={`flex items-center gap-2 sm:gap-3 w-full min-w-0 ${hasExtraContent ? 'border-b border-black/5 pb-2.5' : ''}`}>
                    ...
                    <div className="flex-1 min-w-0 overflow-hidden">
                        ...
                        <p className="font-bold text-sm text-slate-800 line-clamp-2 wrap-break-word leading-tight">
                ...
                <div className={`flex items-center gap-2 sm:gap-3 w-full min-w-0 ${hasExtraContent ? 'border-b border-black/5 pb-2.5' : ''}`}>
                    <div className="w-12 h-12 rounded-full bg-slate-100 flex items-center justify-center text-slate-400 font-bold border-2 border-dashed border-slate-300 shrink-0">
            ...
            {notaTrim && (
                <div className="flex gap-2.5 p-3 bg-amber-50/80 rounded-2xl border border-amber-200 w-full min-w-0">
                    <span className="text-base shrink-0 leading-none mt-0.5" aria-hidden>📝</span>
                    <FormattedNote
                        text={notaTrim}
                        className="text-[13px] sm:text-sm font-medium leading-snug text-amber-800 min-w-0"
                    />
                </div>
            )}
            ...
            {temaIntroduccionAlabanza && (
                <div className="flex items-center justify-center gap-2 sm:gap-3 p-2.5 sm:p-3 bg-blue-50/80 rounded-2xl border border-blue-100 w-full min-w-0">
                    <BookMarked className="w-4 h-4 sm:w-5 sm:h-5 text-blue-600 shrink-0" />
                    <p className="text-sm sm:text-base font-black text-blue-700 leading-tight text-center line-clamp-2">
            ...
            {lecturasRegistradas.length > 0 && (
                <div className="flex flex-col gap-2 p-3.5 bg-white/40 rounded-2xl border border-white/60 shadow-sm relative group w-full min-w-0">
                    <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0 flex-1 flex flex-col justify-center">
                            <div className="flex items-center flex-wrap gap-1.5 mb-1">
                                <span className="text-[9px] font-black uppercase tracking-widest text-blue-600">{lecturasRegistradas.length > 1 ? t('dashboard.himnario.readings') : t('dashboard.himnario.reading')}</span>
                                ...
                                <div className="flex items-center gap-1 bg-emerald-500/10 px-1.5 py-0.5 rounded-md border border-emerald-500/20">
                                    ...
                                    <span className="text-[8px] font-bold uppercase tracking-wider text-emerald-600 whitespace-nowrap">{t('dashboard.himnario.registered')}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div className="space-y-1">
                        {lecturasRegistradas.map((item, idx) => (
                            <div key={item.id ?? `${item.libro}-${item.capitulo_inicio}-${item.versiculo_inicio}-${idx}`} className="flex items-center justify-between gap-2">
                                <p className="font-black text-[13px] sm:text-sm text-slate-800 leading-tight">
                                ...
                                {onEditReading && (
                                    <button
                                        ...
                                        className="shrink-0 w-7 h-7 rounded-full border border-blue-200 bg-blue-50 text-blue-600 hover:bg-blue-100 transition-colors flex items-center justify-center"
            ...
            {himnario && himnario.length > 0 && (
                <div className={`flex flex-col gap-3 p-3 sm:p-4 bg-white/40 rounded-2xl border border-white/60 shadow-sm relative w-full ${lectura ? 'mt-1' : ''}`}>
                    ...
                    <div className="flex items-center gap-3 pb-3 border-b border-black/5">
                        ...
                        <div className="flex-1">
                            <h3 className="text-[10px] font-black uppercase tracking-widest text-indigo-600 leading-none">{himnarioTitle}</h3>
                    ...
                    {allCoros && (
                        <div className="flex items-center justify-start mb-1.5">
                            <span className="text-[9px] font-black uppercase tracking-[0.2em] px-2 py-1 rounded-full bg-purple-50 text-purple-600 border border-purple-200">
                                {t('himnoCoro.choruses')}
                            </span>
                        </div>
                    )}
                    <div className="space-y-1.5 sm:space-y-2">
                        {(himnarioOrdenado ?? himnario ?? []).map((item, idx) => {
                            ...
                            return (
                                <div key={idx} className="flex items-center gap-2 sm:gap-3 group/item p-2 hover:bg-white/50 rounded-xl transition-all min-w-0 w-full">
                                        ...
                                        <div className="min-w-0 flex-1 flex items-center gap-2">
                                            {!allCoros && (
                                                <div className="flex items-center gap-1.5 shrink-0">
                                                    <span className={`text-[8px] font-black uppercase tracking-tighter px-1.5 py-0.5 rounded ${item.tipo === 'himno' ? 'text-indigo-600 bg-indigo-50 border border-indigo-100' : 'text-purple-600 bg-purple-50 border border-purple-100'}`}>
                                                        {item.tipo}
                                                    </span>
                                                </div>
                                            )}
                                            <p className="font-bold text-slate-800 text-xs sm:text-sm leading-tight break-words group-hover/item:text-blue-600 transition-colors">
                                                {details.titulo}
                                            </p>
                                        </div>

                                    <div className="flex items-center gap-1.5 px-2 py-1 bg-slate-100 rounded-lg border border-black/5 shadow-sm shrink-0">
                    ...
                    <div className="flex items-center justify-center mt-2 pt-3 border-t border-indigo-200/50">
                        <div
            ...
            {hasFooter && (
                <div className="w-full pt-3 border-t border-black/5 flex justify-center">
                    {footerAction}
                </div>
            )}
        </div>
